 ...
 * @author Quang
 */
@WebServlet(name = "SearchUserController", urlPatterns = {"/admin/search-user"})
public class SearchUserController extends AdminHomeController {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        //get all users
        int total_user = u.countPagintionUserSearch(Integer.parseInt(role), Integer.parseInt(status),keyword, sort);
        int endPage = total_user / 5;
        if (total_user % 5 != 0) {
            endPage++;
        }
        List<User> users = u.getSearchPagintionUserList(Integer.parseInt(index), Integer.parseInt(role), Integer.parseInt(status),keyword, sort);
