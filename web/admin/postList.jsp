...
<%@page contentType="text/html" pageEncoding="UTF-8"%>
    <script type="text/javascript">
        ...
        function changeStatusFeature(id, items, property)
        {
            alert("Want to change status?");
            window.location.href = "../post/change?id=" + id + "&&items=" + items + "&&property=" + property + "&&index=" + ${requestScope.current_index}
                    ...
                    <div class="panel">
                        <header class="panel-heading">
                            <div class="panel-heading" style="display: flex; justify-content : space-between">
                                <div>Post List</div>
                                <div>
                                    <a href="addpost" style="margin-left: 90px">Add Post</a>
                                </div>
                            </div>
                        </header>
                        <div class="panel-body table-responsive" >
                            <div class="box-tools m-b-15" style="display: flex; justify-content: space-between">
                                ...
                                <form method="" id="filter" action="postlist">
                                ...
                                <form action="searchpost" method="" >
                                    <div class="search">
                                        <input type="text" name="keyword" placeholder="Enter title.."/>
                                        <input name="status" value="${requestScope.status}" style="display: none">
                                        <input name="category" value="${requestScope.category}" style="display: none">
                                        <input name="author" value="${requestScope.author}" style="display: none">
                                        <input name="sort" value="${requestScope.sort}" style="display: none">
                            ...
                            <table class="table table-hover" style="table-layout: fixed;width: 100%">
                                <tr >
                                    ...
                                    <c:forEach items="${requestScope.blogList}" var="bl">
                                        <tr>
                                        <td style="text-align: center" onclick="editPost('${bl.id}')">${bl.id}</td>
                                        ...
                                        <td onclick="editPost('${bl.id}')" style="text-align: center"><b>${bl.category_id.name}</b></td>
                                        <td style="text-align: center">${bl.blogdetail.author.fullname}</td>
                                        <td style="text-align : center">
                                            <c:if test="${bl.status eq true}">
                                                <button style="background-color: green;color: white" onclick="changeStatusFeature(${bl.id},'status',1)">Active</button>
                                            </c:if>
                                            <c:if test="${bl.status eq false}">
                                                <button style="background-color: red;color: white" onclick="changeStatusFeature(${bl.id},'status',0)">In Active</button>
                                            </c:if>
                                        </td>
                                        <td style="text-align : center">
                                            <c:if test="${bl.feature eq true}">
                                                <div onclick="changeStatusFeature(${bl.id},'feature',1)">
                                                    <i class="fa fa-flag" style="font-size: 2.5rem;color: green"></i>
                                                    <p>(On)</p>
                                                </div>
                                            </c:if>
                                            <c:if test="${bl.feature eq false}">
                                                <div onclick="changeStatusFeature(${bl.id},'feature',0)">
            ...
            <div class="text-center">
                <ul class="pagination">
                    <c:if test="${requestScope.current_index + 1 > 1 && requestScope.current_index + 1 < requestScope.endPage}">
                                 <li class="page-item"><a class="page-link" href="?index=0&&status=${requestScope.status}&&category=${requestScope.category}&&author=${requestScope.author}&&sort=${requestScope.sort}">First</a></li>
                    </c:if>
                    <c:if test="${requestScope.current_index != 0}">
                      <li class="page-item">
                          <a class="page-link" href="?index=${requestScope.current_index - 1}&&status=${requestScope.status}&&category=${requestScope.category}&&author=${requestScope.author}&&sort=${requestScope.sort}" aria-label="Previous">
                    ...
                    <c:forEach begin="${requestScope.current_index + 1}" end="${requestScope.current_index + 4}"  var="i">   
                        <c:if test="${i < requestScope.endPage}">
                            ...
                            <li class="${requestScope.current_index == i - 1 ? "page-item active":"page-item"}"><a class="page-link" href="?index=${i-1}&&status=${requestScope.status}&&category=${requestScope.category}&&author=${requestScope.author}&&sort=${requestScope.sort}">${i}</a></li>                                             
                        </c:if>             
                    </c:forEach>
                    <c:if test="${requestScope.current_index != requestScope.endPage - 1}">
                      <li class="page-item">
                      <a class="page-link" href="?index=${requestScope.current_index + 1}&&status=${requestScope.status}&&category=${requestScope.category}&&author=${requestScope.author}&&sort=${requestScope.sort}" aria-label="Next">
